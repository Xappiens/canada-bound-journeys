      ...
      <div className="relative z-10 bg-white/90 backdrop-blur-sm rounded-2xl p-4 sm:p-8 w-11/12 max-w-2xl shadow-xl animate-fade-in my-2 sm:my-20">
        ...
        <div className="flex justify-center mb-4">
          <Link to="/" className={`flex ${isMobile ? 'flex-row' : 'flex-col'} items-center gap-2`}>
            <div className={`${isMobile ? 'w-8 h-8' : 'w-10 h-10'} text-red-600`}>
            ...
            <span className={`${isMobile ? 'text-base' : 'text-lg'} font-bold text-canada-lake`}>Canada BC Experience</span>
          </Link>
        </div>
          ...
          <div className="mb-3 sm:mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
              {seasons.map((season) => (
                ...
                >
                  <span className={`bg-gray-100 w-6 h-6 sm:w-10 sm:h-10 rounded-full flex items-center justify-center mb-1 sm:mb-2 ${
                    selectedSeason === season.id ? "bg-canada-lake/20" : ""
                  }`}>
                    {getSeasonIcon(season.id)}
                  </span>
                  <span className={`font-medium text-sm sm:text-base ${selectedSeason === season.id ? "text-canada-lake" : ""}`}>{season.name}</span>
                  <span className="text-xs text-gray-500 hidden sm:block">{season.months}</span>
                </button>
              ))}
            </div>
        ...
        {step === 'region' && (
          <div className="mb-3 sm:mb-8">
            ...
            <h3 className="text-base sm:text-lg font-medium text-gray-700 mb-2 sm:mb-3">¿Qué región de British Columbia?</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
              {regions.map((region) => (
                ...
                >
                  <h4 className={`font-medium text-sm sm:text-base ${selectedRegion === region.id ? "text-canada-lake" : ""}`}>{region.name}</h4>
                  <p className="text-xs text-gray-600 line-clamp-1">{region.mainAreas.join(", ")}</p>
                </button>
              ))}
            </div>
            ...
            <div className="mt-3 sm:mt-8 text-center">
              <div className="flex flex-row justify-center gap-2 sm:gap-4">
                <Button
                  ...
                  variant="outline"
                  className="border-canada-lake text-canada-lake hover:bg-canada-lake/10 px-3 sm:px-4 py-1 sm:py-2 rounded-md text-sm"
                >
                  Volver
                </Button>
                <Button
                  onClick={onShowItinerary}
                  className="bg-canada-lake hover:bg-canada-lake/90 text-white px-3 sm:px-8 py-1 sm:py-2 rounded-md text-sm"
                >
                  Ver Itinerario
                </Button>
              </div>
        ...
        {/* Show All Itineraries button - Always visible, aligned horizontally on mobile */}
        <div className="text-center mt-2 sm:mt-4">
          <Button
            onClick={onShowAllItineraries}
            variant="outline"
            className="border-canada-lake text-canada-lake hover:bg-canada-lake/10 px-3 sm:px-8 py-1 sm:py-2 rounded-md w-full sm:w-auto text-sm"
          >
            <Grid className="mr-1 sm:mr-2 h-4 w-4" />
